...
//    if(map[X][Y]=0)return;
    if(map[X][Y]=1){ // 1 = +
        if(X-1>=0 && map[X-1][Y]!=0 && visited[X-1][Y]==0)fill(X-1,Y,i+1);
        if(X+1<M && map[X+1][Y]!=0 && visited[X+1][Y]==0)fill(X+1,Y,i+1);

        if(Y-1>=0 && map[X][Y-1]!=0  && visited[X][Y-1]==0)fill(X,Y-1,i+1);
        if(Y+1<N && map[X][Y+1]!=0 && visited[X][Y+1]==0)fill(X,Y+1,i+1);
    }
    else if(map[X][Y]=2){ //2 = |
        if(Y-1>=0 && map[X][Y-1]!=0 && visited[X][Y-1]==0)fill(X,Y-1,i+1);
        if(Y+1<N && map[X][Y+1]!=0 && visited[X][Y+1]==0)fill(X,Y+1,i+1);
    ...
    if(map[X][Y]=5){ //5=F
        if(Y+1<N && map[X][Y+1]!=0 && visited[X][Y+1]==0)fill(X,Y+1,i+1);
        if(Y+1<N && X+1<M && map[X+1][Y+1]!=0 && visited[X+1][Y+1]==0)fill(X+1,Y+1,i+1);
    }
    if(map[X][Y]=6){ // 6 = 7
        if(Y+1<N && map[X][Y+1]!=0 && visited[X][Y+1]==0)fill(X,Y+1,i+1);
        if(Y+1<N && X-1>=0 && map[X-1][Y+1]!=0 && visited[X-1][Y+1]==0)fill(X-1,Y+1,i+1);
    }
    if(map[X][Y]=7){ // 7 = J
        if(Y+1<N && map[X][Y+1]!=0  && visited[X][Y+1]==0)fill(X,Y+1,i+1);
        if(X-1>=0 && map[X-1][Y]!=0 && visited[X-1][Y]==0)fill(X-1,Y,i+1);
