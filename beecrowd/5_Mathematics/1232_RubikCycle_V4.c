...
int main()
{
    char stringArray[10000000][MAX_LENGTH];
    int i;

//  Input from users
    for (i = 0; i >= 0; i++)
    {
        fgets(stringArray[i], MAX_LENGTH, stdin);
        if (stringArray[i][0] == '\n')   // stop taking input when an empty line is entered
        {
            break;
        }
    }

    // Initiallize Rubik Cube;
    ...
    for (int j = 0; j < i; j++)
    {
        char seq[strlen(stringArray[j])];
        strcpy(seq, stringArray[j]);
//        printf("%s", seq);
